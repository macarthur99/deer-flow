...
class FileMemoryStorage(MemoryStorage):
    ...
    def __init__(self):
        ...
        # Per-user per-agent memory cache: keyed by (user_id, agent_name)
    ...
    def _get_memory_file_path(self, user_id: str, agent_name: str | None = None) -> Path:
        ...
        if agent_name is not None:
            ...
            # Per-user per-agent: memory/{user_id}/agent_{agent_name}.json
            return base_dir / user_id / f"agent_{agent_name}.json"

        # Per-user: memory/{user_id}/memory.json
        return base_dir / user_id / "memory.json"

    def _load_memory_from_file(self, user_id: str, agent_name: str | None = None) -> dict[str, Any]:
        """Load memory data from file."""
        file_path = self._get_memory_file_path(user_id, agent_name)
    ...
    def load(self, user_id: str, agent_name: str | None = None) -> dict[str, Any]:
        ...
        if cached is None or cached[1] != current_mtime:
            memory_data = self._load_memory_from_file(user_id, agent_name)
